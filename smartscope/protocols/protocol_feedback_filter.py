...
class smartscopeFeedbackFilter(ProtImport, ProtStreamingBase):
    ...
    def stepsGeneratorStep(self):
        ...
        while not self.finish:
            ...
            if rTime >= self.refreshTime.get():
                self.zeroTime = time.time()
                self.info('len filteredMics: {}'.format(len(self.filteredMics.get())))
                if len(self.filteredMics.get()) >= self.triggerMicrograph.get():
                    ...
                    self.info('Waiting enought micrographs to launch protocol.'
                              ' triggerMicrograph: {}, micrographsFiltered: {}'.format(self.triggerMicrograph.get(), len(self.filteredMics.get())))
    ...
    def statistics(self):
        ...
        mu, sigma, minIntensityL, maxIntensityL = calculate(listRange[np.argmax(shapiroList)])
        # SUMMARY INFO
